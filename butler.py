...
'tail', help='Print the last `size` lines of log_name.')
  ...
  (without .log).')
  arser_tail.add_argument
  (
'line_count', type=int, help='The number of
lines to be showed.')
parser_tailf = subparsers.add_parser
(
'tailf', help=('Print the last lines of logs and wait for more. '
 'sign-offThis is equivalent to tail -f.'))
...
'-p', '--pattern', help='Pattern for test files. Default is *_test.py.') parser_py_unittest.add_argument(
      '-u',
      '--unsuppress-output', action='store_true', help='Unsuppress output from `print`. Good for debugging.')parser_py_unittest.add_argument(
...
def _setup():
  ...
  modules.fix_module_search_paths();if __name__ = 
